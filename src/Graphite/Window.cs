...
namespace Graphite
{
    public class Window
    {
        ...
        public void ConsumeRelease(StylusState stylus, DigitizerEventKeyCode code)
        {
            ...
            Console.WriteLine(
                "Release(" + stylus.DevicePosition.X + ", " + stylus.DevicePosition.Y + ") " + 
                (eligibleControls.Any() ? eligibleControls.Count().ToString() : "no") + " control at position" );
        ...
        public async Task<Page> CreatePage(Task<Panel> panel)
        {
            return new Page(this, Width, Height, await panel);
        }
